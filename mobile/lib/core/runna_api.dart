...
class RunnaApi {
  ...
  Future<List<ManualRouteItem>> getManualRoutes(String accessToken) async {
    final response = await _client.get(_uri('/map/manual-routes'), headers: _jsonHeaders(accessToken));
    _ensureSuccess(response);
    final body = jsonDecode(response.body) as List<dynamic>;
  ...
  Future<List<RunItem>> getRuns(String accessToken) async {
    final response = await _client.get(_uri('/runs'), headers: _jsonHeaders(accessToken));
    _ensureSuccess(response);
    final body = jsonDecode(response.body) as List<dynamic>;
    return body.map((item) => RunItem.fromJson(item as Map<String, dynamic>)).toList();
  }

  Future<RunItem> getRun({required String accessToken, required int runId}) async {
    final response = await _client.get(_uri('/runs/$runId'), headers: _jsonHeaders(accessToken));
    _ensureSuccess(response);
  ...
  Future<AdminStats> getAdminStats(String accessToken) async {
    final response = await _client.get(_uri('/admin/stats'), headers: _jsonHeaders(accessToken));
    _ensureSuccess(response);
  ...
  Future<List<AdminUserItem>> getAdminUsers(String accessToken) async {
    final response = await _client.get(_uri('/admin/users'), headers: _jsonHeaders(accessToken));
    _ensureSuccess(response);
    final body = jsonDecode(response.body) as List<dynamic>;
  ...
  Future<List<HazardMarkerItem>> getAdminMarkers(String accessToken) async {
    final response = await _client.get(_uri('/admin/markers'), headers: _jsonHeaders(accessToken));
    _ensureSuccess(response);
    final body = jsonDecode(response.body) as List<dynamic>;
    return body.map((item) => HazardMarkerItem.fromJson(item as Map<String, dynamic>)).toList();
  }
